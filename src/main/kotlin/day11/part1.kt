...
fun main() {
    ...
    val expanded = expand1(lines)
//    printMap(expanded)
    val points = findPoints1(expanded)
//    println(points)
    var sum = 0
    for (point1Index in points.indices) {
        for (point2Index in (point1Index..<points.size)) {
            sum += getDistance1(points[point1Index], points[point2Index])
        }
    }
    println("Part 1: $sum")
}

fun getDistance1(point1: Point1, point2: Point1): Int {
...
fun findPoints1(lines: List<String>): List<Point1> {
...
fun expand1(lines: List<String>): List<String> {
...
fun printMap1(lines: List<String>) {
    for (line in lines) {
        for (col in line) {
            print(col)
        }
